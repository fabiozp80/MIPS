.data
bienvenida: .asciiz "VAMOS A COMPARAR TRES NUMEROS Y ENCONTRAR EL MAYOR"       # Define una cadena de bienvenida.
captura: .asciiz "Por favor, ingrese un numero: "                              # Define una cadena que se utilizará para pedir al usuario un número.
resultado: .asciiz "El numero mayor es: "                                      # Define una cadena que precederá al resultado.
...
main:
    ...
    bgt $t0, $t1, compara_tercero          # Compara los registros $t0 y $t1; si $t0 es mayor, salta a compara_tercero.
    move $t0, $t1                          # Si $t1 es mayor, mueve $t1 a $t0.

compara_tercero:			   # Bloque compara_tercero para comparar el que ahora es el mas grande $t0 con el $t2 (tercero)
    bgt $t0, $t2, muestra_resultado        # Compara $t0 con $t2; si $t0 es mayor, salta a muestra_resultado.
    move $t0, $t2                          # Si $t2 es mayor, mueve $t2 a $t0.

muestra_resultado:			   # Bloque muestra resultado, para mostrar en pantalla el numero que resulto se mayor de los 3
